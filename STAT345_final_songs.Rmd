...
```{r}
##Scrape Wikipedia as it is easier? https://en.wikipedia.org/wiki/Billboard_Year-End_Hot_100_singles_of_2020
...
#Cleans data
songs_2020 <- songs_2020 %>%
  select(Title, `Artist(s)`) %>%
  rename(title = Title, artist = `Artist(s)`) %>%
  mutate(year = 2020)
